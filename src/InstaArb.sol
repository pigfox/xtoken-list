...
contract InstaArb is Ownable(msg.sender) {
    ...
    function addStables(address[] calldata _stables) external onlyOwner {
        for (uint256 i = 0; i < _stables.length; i++) {
    ...
    function estimateDualDexTrade(address _router1, address _router2, address _token1, address _token2, uint256 _amount)
        external
        view
        returns (uint256)
    {
    ...
    */
    function instaSearch(address _router, address _baseAsset, uint256 _amount)
        external
        view
        returns (uint256, address, address, address)
    {
        ...
        address token2;
        address token3;
        for (uint256 i1 = 0; i1 < tokens.length; i1++) {
            for (uint256 i2 = 0; i2 < stables.length; i2++) {
                for (uint256 i3 = 0; i3 < tokens.length; i3++) {
